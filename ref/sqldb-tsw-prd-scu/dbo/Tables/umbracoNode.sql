...
GO
CREATE NONCLUSTERED INDEX [IX_umbracoNode_ObjectType_trashed_sorted]
    ON [dbo].[umbracoNode]([nodeObjectType] ASC, [trashed] ASC, [sortOrder] ASC, [id] ASC)
    INCLUDE([uniqueId], [parentId], [level], [path], [nodeUser], [text], [createDate]);


GO
CREATE NONCLUSTERED INDEX [IX_umbracoNode_parentId_nodeObjectType]
    ON [dbo].[umbracoNode]([parentId] ASC, [nodeObjectType] ASC)
